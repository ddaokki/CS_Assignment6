...
vec3 computeFlatShading(const vec3 & pos, const vec3 & normal) {
    ...
    vec3 lightColor = vec3(1.0f); // white
    vec3 ambient = Ia * ka;

    vec3 L = normalize(lightPos - pos);
    vec3 V = normalize(-pos); // viewer at origin
    ...
    color = clamp(color, 0.0f, 1.0f);      // 마지막에 clamp
...
void rasterize(const vector<vec3>&screenVerts, const vector<vec3>&worldVerts) {
    for (const auto& tri : triangles) {
        ...
        // Normal and centroid for flat shading
        vec3 normal = normalize(cross(w1 - w0, w2 - w0));
        vec3 centroid = (w0 + w1 + w2) / 3.0f;
        vec3 viewDir = normalize(-centroid);
        if (dot(normal, viewDir) < 0.0f) normal = -normal;

        vec3 color = computeFlatShading(centroid, normal);
...
void createSphere() {
    int w = 24; // 경도 방향 삼각형 수
    int h = 12;  // 위도 방향 삼각형 수


    // 중간 정점 (위도 h-2개 층, 경도 w개씩)
    for (int j = 1; j < h - 1; ++j) {
        float theta = (float)j / (h - 1) * M_PI;
        for (int i = 0; i < w; ++i) {
            float phi = (float)i / w * 2 * M_PI;  // wrap-around를 위해 w로 나눔
    ...
    vertices.push_back(vec3(0, 1, 0)); // top pole

    int south = vertices.size();
    vertices.push_back(vec3(0, -1, 0)); // bottom pole
    ...
    for (int j = 0; j < h - 3; ++j) {
        for (int i = 0; i < w; ++i) {
            ...
            triangles.push_back({ curr, belowNext, next }); // 시계 반대 방향
    ...
    for (int i = 0; i < w; ++i) {
        int next = (i + 1) % w;
        triangles.push_back({ north, i, next });
    }
    ...
    for (int i = 0; i < w; ++i) {
        int next = (i + 1) % w;
        triangles.push_back({ south, base + next, base + i });
    }
}

void render() {
    createSphere();
    vector<vec3> screenVerts, worldVerts;
    for (const auto& v : vertices) {
        vec3 model = transformModel(v);
    ...
    rasterize(screenVerts, worldVerts);
}

int main() {
    if (!glfwInit()) return -1;
    GLFWwindow* window = glfwCreateWindow(width, height, "Q1 - Flat Shading", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
