...
void rasterize(const vector<vec3>& screenVerts, const vector<vec3>& worldVerts, const vector<vec3>& vertexColors) {
    for (const auto& tri : triangles) {
        vec3 v0 = screenVerts[tri.v0];
        vec3 v1 = screenVerts[tri.v1];
        vec3 v2 = screenVerts[tri.v2];

        vec3 c0 = vertexColors[tri.v0];
        vec3 c1 = vertexColors[tri.v1];
        vec3 c2 = vertexColors[tri.v2];
...
void render() {
    ...
    rasterize(screenVerts, worldVerts, vertexColors);
}

int main() {
    if (!glfwInit()) return -1;
    GLFWwindow* window = glfwCreateWindow(width, height, "Q2 - Gouraud Shading", NULL, NULL);
    if (!window) {
